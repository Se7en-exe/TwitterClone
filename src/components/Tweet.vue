...
export default {
  ...
  methods: {
    ...
    async randomUser() {
      await axios
        ...
        .then((response) => {
          ...
          // 设置作者为随机用户的全名
          this.tweetBody.author = `${this.userData.firstName} ${this.userData.lastName}`;
        })
        .catch((error) => {
          console.log(error);
        });
    ...
    async getTweets() {
      await axios.get("http://localhost:5174/api/tweets?count=12")
        .then((response) => {
          const tweetData = response.data[0]; // 获取第一条推文
          this.tweetBody.content = tweetData.content; // 设置推文内容
          // 不再设置推文作者，这里将使用 randomUser 设置的作者
          // this.tweetBody.author = tweetData.author; // 删除这行
          this.likeNumber = Math.floor(Math.random() * 100);
          this.reTweetNumber = Math.floor(Math.random() * 100);
          this.replyNumber = Math.floor(Math.random() * 100);
          this.date = Math.floor(Math.random() * 24) + "h"; // 生成随机时间
        })
        .catch((error) => {
          console.log(error);
        });
  ...
  async created() {
    await this.randomUser(); // 先获取随机用户
    await this.getTweets(); // 然后获取推文
  },
};
</script>
